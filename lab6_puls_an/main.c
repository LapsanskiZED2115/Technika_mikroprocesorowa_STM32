...
HAL_ADC_Start_DMA(&hadc1, (uint32_t*)ADCValue, 2);
  ...
  {
	  //cute com
//filtr usredniajacy, wlasciwosci dolnoprzepustowe
	  //caly czas sumowac i jak dojdziemy do ilus to podzelic i sume wyzerowac
	  //przesuniecie bitowe zamist dzielenia

//64 elementy do srednij i shiftowanie o 6 bitow w prawo



	//	TxSize = snprintf(TxBuff, sizeof(TxBuff), "hhh %d, \n", 10);
	//	HAL_UART_Transmit_DMA(&huart2, TxBuff, TxSize);
	//  HAL_Delay(25);

	  if(ADC_flag_Ready == 1)
	  {
		  uint16_t TxSize = snprintf(buf_tx, sizeof(buf_tx), "%d, %d\n", ADCValue[0], ADCValue[1]);
